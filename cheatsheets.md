...
### 5.4 CEPH NVMe-oF Gateway Setup
...
# 5. Verify gateway
ceph orch ps
gwcli.py subsystem list
```

### 5.5 GFS2 Cluster Setup

```bash
# 1. Install GFS2 packages
apt-get install gfs2-utils gfs2-cluster dlm

# 2. Configure Pacemaker
pcs cluster setup --name mycluster node1 node2 node3

# 3. Create GFS2 resource
...
rbd snap rollback <pool>/<image> --snap <snap-name>
```

### 9.3 CephFS Operations
...
## 10. CEPH iSCSI/NVMe-oF/GFS2 Troubleshooting

### 10.1 iSCSI Troubleshooting
